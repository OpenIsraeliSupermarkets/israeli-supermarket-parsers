...
class TestCSVOutputWriter(unittest.IsolatedAsyncioTestCase):
    ...
    def _new_writer(
        self, reduce_duplicates: bool = True
    ) -> CSVOutputWriter:
        return CSVOutputWriter(
            self._output_folder, "shufersal", "pricefull", reduce_duplicates=reduce_duplicates
        )
    def _csv_path(self) -> str:
        return os.path.join(self._output_folder, "shufersal_pricefull.csv")
    def _read_data_rows(self) -> list[dict[str, str | None]]:
        with open(
            self._csv_path(), newline="", encoding="utf-8"
        ) as f:
    ...
    async def test_initialize_reads_existing_file_columns(self) -> None:
        ...
        await writer.initialize()
        self.assertEqual(
            writer._existing_columns, ["x", "y", "z"]  # pylint: disable=protected-access
        )
